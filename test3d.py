...
weightLine, = plt.plot(x, perceptron.calculate_boundary(x), '-r', label='y=2x+1',color = 'black', linewidth=3)



def fireResult(weights,dataPoints,finished):
    if not finished:
        selectPlot.set_ydata([dataPoints[1]])
        selectPlot.set_xdata([dataPoints[0]])
        weightLine.set_xdata(x)
        weightLine.set_ydata(perceptron.calculate_boundary(x))
    else:
        x0, x1 = np.meshgrid(
            np.linspace(-10.0, 10.0, int((10.0 - (-10.0)) * 100)),
            np.linspace(-10.0, 10.0, int((10.0 - (-10.0)) * 100)),
            )
...
def _runPerceptron(): 
    perceptron.data = np.array(data)
    perceptron.solve()
    # dataNP = 
perceptron.handlers = [fireResult]
...
algoButton = tkinter.Button(master=root, text="Run Perceptron", command=_runPerceptron,state=tkinter.DISABLED)
